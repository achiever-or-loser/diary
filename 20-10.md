...
beanDefine 有3种role：application
...

{
	...
	"BlockInfoList": [{
			...
			"Code": "001",
			"IntroduceType": "1",
			"IntroduceTxt": "汽车产业链 基本介绍_文本",
			"IntroduceImgUrl": "汽车产业链基本介绍_图片",
			"LogicType": "0",
			"LogicTxt": "汽车产业链投资逻辑_文本",
			"LogicImgUrl": "汽车产业链投资逻辑_图片",
			"Type": "2",
			"BlockList": "002,003"
		},
		{
...
http://61.152.230.66/emis-job-admin/

http://61.152.230.66:443/emis-job-admin/

http://61.152.230.66/emis-job-admin/toLogin

http://61.152.230.66:443/emis-job-admin/toLogin
...
~~~
ARG REPO=10.205.104.109:9527
ARG JRE=$REPO/emis/java/openjdk:8u212-alpine-3
FROM $JRE
WORKDIR /app
COPY target/taskScheduling.jar taskScheduling.jar
ENTRYPOINT ["java", "-jar","taskScheduling.jar","--spring.config.location=/data/java/config/application-baseConfig.properties"]
~~~



~~~
docker build -t 10.205.104.109:9527/emis_data_scheduling:v1 ./
# 如果已经有container在运行，删掉
output=$(docker ps -a |grep emis_data_scheduling)
echo $output
if [ ! -n "$output" ]; then
    echo "container not exit"
else
    docker stop emis_data_scheduling
    docker rm emis_data_scheduling
fi

docker run --restart=always \
-itd --name emis_data_scheduling \
-p 8107:8107 \
-v /home/hanqingrui/webApplications/taskSchedulingApi/config:/data/java/config \
10.205.104.109:9527/emis_data_scheduling:v1
~~~
